...
<?php
	...
	function leeNivel($objeto,$nivelTAb) { 
		foreach ($objeto as $key => $value) {
			if ( $key == 'EntradaGraficaLIDER' ) {
				echo "<br>\n========================================= ESTOY EN EntradaGraficaLIDER\n";


				$lider = explode("\n", $value);		//SI FUNCIONA crea un array con las lineas

				//~ $lider = parse_ini_string ($value, true);
				//~ echo print_r($lider);
				//~ echo json_encode($lider);
				echo "	<script>	var lider		= ".json_encode ($lider).";	console.log( lider);	</script>\n		";
				foreach($lider as $linea) {
					echo "<br>$linea";
				}
				...
				//~ echo str_replace (array("\r\n", "\n", "\r"), '<br>', $value);	//SI FUNCIONA  muestras las lineas
				//~ echo $value;
	...
	if (isset ($archivoGET) ) {
		...
		$xml = simplexml_load_file("archivos/".$archivoGET.".ctehexml"); // or die("Error:  Xml file load failure");
		...
		echo "	<script>	console.log( archivoGET);	</script>		";

		echo "	<script>	var xml		= ".json_encode ($xml).";	</script>		";
		...
		foreach (glob("archivos/*.ctehexml") as $nombre_fichero) {
